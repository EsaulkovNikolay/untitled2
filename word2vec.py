...
class Word2vec:
    ...
    def __filter_input(self, test_input):
        no_input = []
        real_input = []
        for c in test_input:
    ...
    def test_1(self, report_file_name=None, topn=5):
        ...
        for i, test in enumerate(test_values):
            ...
            if not test_input == test_answer:
                ...
                real_input = self.__filter_input(test_input)

                similarities = self.model.most_similar(positive=real_input, topn=topn)

                result = list(map(lambda x: x, filter(lambda x: x[0] in self.values, similarities)))

                result = [c for c in result if any([str(c[0]).find(x) != -1 for x in real_input])]

                result_len += len(result)
                max_result_len = max(max_result_len, len(result))

                if test_answer in list(map(lambda x: x[0], result)):
                    passed_count += 1
            else:
                passed_count += 1
                easy_passed_count += 1
        ...
        for i in range(0, tests_len):
            try:
                ...
                if not test_values[i][0].upper() == test_values[i][1].upper():
                    similarities = self.model.most_similar(positive=real_input, topn=5)
    ...
    def find(self, search_input):
        ...
        similarities = self.model.most_similar(real_input, topn=10)
        result = []
        if len(similarities):
            result = list(map(lambda x: [x[0], round(x[1], 3)], filter(lambda x: x[0] in self.values, similarities)))
